...
public class AuthController {
        ...
        @Operation(
                ...
                description = "Valida las credenciales del usuario (Email y Contraseña). Si son correctas, genera un JSON Web Token (JWT) firmado con algoritmo HMAC-SHA que debe adjuntarse en las cabeceras subsecuentes."
        )
        @ApiResponse(
                responseCode = "200", 
        ...
        @ApiResponse(
                responseCode = "401", 
                description = "Credenciales inválidas (Usuario o contraseña incorrectos) o cuenta deshabilitada/revocada administrativamente.",
        ...
        @Operation(
                ...
                description = "Revoca el token JWT actual del usuario, invalidándolo para futuras peticiones. Requiere que se envíe el token en la cabecera Authorization.",
        ...
        public ResponseEntity<?> logout(
                        @Parameter(description = "Token JWT de tipo Bearer", required = true, example = "Bearer eyJhbGciOiJIUzI1NiIsInR5c...")
        ...
        @Operation(
                ...
                description = "Verifica si el token proporcionado en la cabecera es válido, no ha expirado y no ha sido revocado. Retorna los datos básicos de la sesión (usuario y roles) si es válido.",
                security = @SecurityRequirement(name = "bearerAuth")
        )
        @ApiResponse(
                responseCode = "200", 
                description = "Retorna el estado de validación del token. Puede indicar si es válido, expirado o revocado.",
                content = @Content(
                        mediaType = "application/json",
                        examples = {
                                @ExampleObject(name = "Token Válido", value = "{\n  \"valid\": true,\n  \"message\": \"Token es válido\",\n  \"user\": \"usuario@ejemplo.com\",\n  \"roles\": [\"USER_APP\"]\n}"),
                                @ExampleObject(name = "Token Expirado", value = "{\n  \"valid\": false,\n  \"error\": \"Token ha expirado\",\n  \"expired\": true\n}"),
                                @ExampleObject(name = "Token Revocado", value = "{\n  \"valid\": false,\n  \"error\": \"Token ha sido revocado\",\n  \"revoked\": true\n}"),
                                @ExampleObject(name = "Token No Encontrado", value = "{\n  \"valid\": false,\n  \"error\": \"Token no encontrado en la base de datos\"\n}")
                        }
                )
        )
        @ApiResponse(
